create database BDProyectoMVC collate Modern_Spanish_CI_AS
go
...
(
    ...
    usuario_vendedor varchar(50)
        constraint Venta_Usuario_usuario_fk
            references Usuario,
    folio            int not null,
...
(
    idVenta  int         not null
        constraint ProductosdeVenta_Venta_idVenta_fk
            references Venta
            on update cascade on delete cascade,
    sku      varchar(20) not null
        constraint ProductosdeVenta_Producto_sku_fk
            references Producto
            on update cascade on delete cascade,
...
create unique index ProductosdeVenta_idVenta_uindex
    on ProductosdeVenta (idVenta)
go

create unique index Venta_folio_uindex
    on Venta (folio)
go
